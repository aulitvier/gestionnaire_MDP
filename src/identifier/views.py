...
class IdentifierView(CreateView): # todo 1 créer la logique qui permet d'enregistrer l'indetinfiants, mdp...
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST: # si le client renvoye soumet le formulaire
            context['child_form'] = ChildModelFormSet(self.request.POST) #ChildModelFormSet est initié avec les données de POST
        else:
            context['child_form'] = ChildModelFormSet() # si le formulaire n'est pas envoyer affiche le formulaire vide
        return context


# REGARDER POURQUOI LA JOINTURE USER ET CUSTOMUSER NE SE FAIT PAS
    # self.request.user CONTIENT BIEN L'UTILISATEUR AVEC SON ID
    # form.instance.User_id RENVOIE UNE ERREUR
    def form_valid(self, form):
        ...
        if self.request.POST:
            ...
            if value_already_exists:
                ...
                # après save, car on peut pas save de formset
...
ChildModelFormSet = inlineformset_factory(Username, Login_informations, form=LoginInformationsForm,extra=1)  # permet d'afficher les deux formulaire
'''
